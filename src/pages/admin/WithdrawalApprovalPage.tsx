...
import { CheckCircle, XCircle, AlertCircle, CreditCard, Smartphone } from 'lucide-react';
...
export default function WithdrawalApprovalPage() {
  ...
  const handleApprove = async (withdrawal: Withdrawal) => {
    ...
    try {
      ...
      await walletBalanceService.updateBalance(withdrawal.userId, {
        earnings: Math.max(0, currentBalance.earnings - withdrawal.amount)
      });
    } catch (error) {
              ...
              <tbody>
                {withdrawals.map((withdrawal) => (
                  ...
                  >
                    ...
                    <td className="p-4 font-semibold">{formatCurrency(withdrawal.amount)}</td>
                    <td className="p-4 text-red-400">{formatCurrency(calculateProcessingFee(withdrawal.amount))}</td>
                    <td className="p-4 text-green-400 font-semibold">{formatCurrency(calculateNetAmount(withdrawal.amount))}</td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      {withdrawal.paymentMethod === 'upi' ? (
                        ...
                        <div className="text-sm space-y-1">
                          <p><strong>Name:</strong> {withdrawal.accountHolderName}</p>
                          <p><strong>Account:</strong> ****{withdrawal.accountNumber?.slice(-4)}</p>
                          <p><strong>IFSC:</strong> {withdrawal.ifscCode}</p>
                        </div>
                      )}
                    </td>
                    <td className="p-4">
