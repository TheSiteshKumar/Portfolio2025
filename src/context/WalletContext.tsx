...
interface WalletContextType {
  ...
  withdraw: (amount: number, withdrawalDetails: any) => Promise<void>;
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const withdraw = async (amount: number, withdrawalDetails: any) => {
    if (!user) throw new Error('User must be logged in to withdraw');
    if (totalEarnings < amount) throw new Error('Insufficient balance');

    try {
      const withdrawalId = await walletService.createWithdrawal(user.uid, amount, withdrawalDetails);
      const newWithdrawal: Withdrawal = {
        ...
        ...(withdrawalDetails.paymentMethod === 'upi' ? {
          ...
          ifscCode: withdrawalDetails.ifscCode,
      ...
      addTransaction({
        ...
        amount,
        timestamp: new Date().toISOString(),
        description: `Withdrawal request submitted via ${withdrawalDetails.paymentMethod.toUpperCase()}`,
      });
    } catch (error) {
