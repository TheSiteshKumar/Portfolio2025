...
export default function PlanDetails() {
  ...
  const handleInvest = () => {
    if (!canPurchase) {
      setError(`Purchase limit reached for this plan. You can only purchase this plan ${plan.purchaseLimit} time${plan.purchaseLimit > 1 ? 's' : ''}.`);
      return;
    }
          ...
          <div className="grid md:grid-cols-2 gap-8">
                ...
                {plan.purchaseLimit > 0 ? (
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="text-gray-300">
                      Purchase limit: {purchaseCount}/{plan.purchaseLimit} used
                    </span>
                  </div>
                ) : (
                  <div className="flex items-center space-x-3">
            ...
            <div className="bg-gray-900 p-6 rounded-xl">
              ...
              <div className="space-y-4 mb-6">
                ...
                {plan.purchaseLimit > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-400">Purchases Remaining</span>
                    <span className="text-blue-400 font-semibold">
                      {plan.purchaseLimit - purchaseCount} of {plan.purchaseLimit}
                    </span>
                  </div>
                )}
              </div>
              ...
              {!canPurchase && (
                <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 mb-4">
                  <p className="text-yellow-400 text-sm">
                    ...
                    You have reached the maximum purchase limit for this plan ({plan.purchaseLimit} time{plan.purchaseLimit > 1 ? 's' : ''}).
                  </p>
                </div>
              )}
